...
## 02/09 (segunda rodada) — mensagem inteira editável, sem o bloco do mês, envio por seleção e uma data só

Quatro pedidos de quem usa a tela, todos olhando o mesmo print: o cartão do
touchpoint final.

### A mensagem inteira virou editável

Até aqui só os três campos de texto (`Como foi`, `Próximo passo`, `O que
precisamos de você`) eram digitáveis. O cabeçalho — investimento, leads, CPL,
agendamento — vinha do contrato e ninguém encostava nele: **era essa a
garantia de que nenhum número da mensagem nascia na tela**.

O botão **Editar a mensagem inteira** troca o cartão por uma caixa só, com a
mensagem completa dentro. Isso abre mão da garantia estrutural, então ela vira
uma **conferência**: `numerosForaDoContrato()` extrai todo número do texto
(descontando datas `dd/mm` e as constantes de prosa declaradas) e procura cada
um dentro do contrato. O que não estiver lá aparece num alerta vermelho e num
item novo do checklist 8.8.

A conferência **avisa, não trava**. Quem pediu a mensagem editável pediu para
poder escrever o que precisa; travar seria devolver a decisão para a máquina
depois de o humano ter pedido o contrário. O que continua travando é o
marcador `[…]`, porque ali a informação realmente não existe — e essa trava
também é do lado do servidor.

Os dois modos convivem sem perder trabalho: ligar o modo livre guarda os três
campos, desligar guarda a mensagem escrita à mão (o cartão avisa que ela
existe), e só **Descartar edição** joga fora. `Escrever o período todo` não
apaga a versão à mão de ninguém.

### O bloco `📊 No mês` saiu da mensagem

Ele imprimia, em número cru, o que a prosa do "Como foi" já diz interpretado
(quanto falta para fechar o mês). E num recorte que atravessa a virada ficava
absurdo: `📊 No mês (01/09 a 01/09): • Leads: 0` logo abaixo de 3 leads em 8
dias.

O mês **continua na tela**, no painel de números — o que ele deixou de ser é
linha publicada. A regra do contrato não mudou (o mês continua sendo o do
último dia do recorte) e `incoerencias()` continua travando o salvar quando o
mês e o período não fecham entre si.

Duas consequências que valem registro:

- a linha `• Agendamentos: N de M contratados` era onde vivia o rótulo
  `contrato` vs. `benchmark` — a correção de 29/08 do `model.ts`. A distinção
  não sumiu: ela mora na prosa, e `testar_correcao.mjs` passou a segurar lá o
  que segurava aqui (benchmark nunca vira "contratado");
- o esqueleto do `app.js` citava o mês sem conferir se ele contém o período —
  a regra que o `redacao.js` já seguia. Agora segue também.

### Enviar vários clientes de uma vez, escolhendo quais

Cada cliente da lista ganhou uma caixa de seleção, e a barra em cima da lista
diz, antes de qualquer clique, **quantos vão no envio**. O botão da barra
superior passou a dizer o número junto: `Revisar e enviar — 27 cliente(s)`.

A regra antiga continua de pé: **o filtro não manda no envio**. Uma busca
esquecida na caixa fazendo um cliente sumir do envio sem ninguém perceber é o
pior resultado possível desta tela. O que manda é a seleção, que é explícita,
aparece marcada e tem os ausentes **nomeados** no diálogo de envio ("2
cliente(s) prontos ficaram FORA desta seleção"). O botão **Só os visíveis** é
a ponte declarada entre as duas coisas: filtre por gestor, clique, e o envio
passa a ser daquele gestor por decisão, não por efeito colateral.

Sem seleção = todos os prontos (o comportamento de sempre). Seleção vazia é
diferente de ausência de seleção: desmarcar todo mundo **desabilita** o envio
em vez de mandar a semana inteira. São campos distintos de propósito, e
`testar_filtros.mjs` segura a diferença.

O diálogo do CS ganhou o terceiro escopo — período todo (ignora a seleção),
**os N selecionados**, ou só o cliente aberto — e abre já marcado no do meio
quando existe seleção manual.

### Um formato de data só

O cabeçalho saía em padrão americano (`Mon, 08/17 to Sun, 08/23`) enquanto
**todo o resto** da mensagem saía em `dd/mm`: a data do "Próximo passo", a de
validar uma otimização, o bloco do mês. Dois formatos na mesma mensagem fazem
`08/09` ser lido como 9 de agosto — número certo, leitura falsa, que é a
família de defeito que originou este projeto.

Agora é `17/08 a 23/08` nas duas pontas. A função vive em dois arquivos sem
import entre eles (`src/contrato.js` e `docs/app.js`); `testar_periodo.mjs`
compara as duas implementações e `testar_app.mjs` afirma que o cabeçalho de
todo bloco casa com `dd/mm a dd/mm`.

### Um defeito de concordância achado no caminho

Escrevendo o teste apareceu `No mês são 1 agendamentos` — mesma classe do
`os 1 lead` corrigido de manhã: trecho montado por concatenação, com o verbo
preso no plural. Corrigido em `redacao.js` (`No mês há 1 agendamento`), junto
com o artigo de `alvoMes`.
...
node testar_periodo.mjs          # o recorte livre: janela, benchmark, texto e o rótulo dd/mm
